#!/usr/bin/env python3
"""
RPD + Taguchi: один скрипт без отдельного «debug»-модуля.

- Полный прогон: все листы из taguchi JSON × RPD_BLOCKS (по умолчанию adaptive,base).
  Переменные: RPD_SHEETS (опционально), RPD_BLOCKS, остальное см. README_RPD.md.

- Один лист: задайте один элемент, например
  RPD_SHEETS=GEA_2 RPD_BLOCKS=adaptive python3 run_rpd_tune_all_sheets.py

Лог основного процесса (включая весь вывод RPD в этом же процессе):
  results/rpd_tune_all_sheets.log
Отключить: RPD_MAIN_LOG=0. Путь: RPD_MAIN_LOG_FILE=...
"""
